...
int proc_block(int pid)
{
        ...
        int result = sched_setscheduler(pid, SCHED_IDLE, &param); // sets scheduling policy and parameters for pid, 0 for success, -1 for error
...
int proc_wakeup(int pid)
{
        struct sched_param param;
        param.__sched_priority = 0;                                 // param->sched_priority must be 0 for SCHED_OTHER, SCHED_BATCH, SCHED_IDLE
        int result = sched_setscheduler(pid, SCHED_OTHER, &param);  // sets scheduling policy and parameters for pid, 0 for success, -1 for error
