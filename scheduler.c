...
void prior_update_sort(struct process* proc, int nprocs) {
    // update priority and sort
    qsort(proc, nprocs, sizeof(struct process), cmp_proc_sjf);
    /*
    for (int id = 0; id < nprocs; id++)
            processes[id].prior_tmp = MAX_PRIOR_PREM - id;
    */
    qsort(proc, nprocs, sizeof(struct process), cmp_proc_fcfs);
}
